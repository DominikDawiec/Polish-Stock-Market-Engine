...
if option:
    ...
    with st.expander("🤖 Forecast"):
     ...
     with tab1:
          ...
          # Select the appropriate number of forecasted values from y_pred
          y_pred_forecast = y_pred[-num_days:]

          # Create a dataframe with the forecasted values and dates
          forecast_df = pd.DataFrame({'Date': forecast_dates, 'Forecasted Close': y_pred_forecast})
